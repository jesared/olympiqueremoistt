  ...
  return (
    <section className="relative isolate flex min-h-[calc(100vh-4rem)] items-center justify-center overflow-hidden px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto w-full max-w-3xl rounded-3xl border border-white/15 bg-white/8 p-8 text-center shadow-2xl backdrop-blur-xl sm:p-12">
        <p className="mb-4 inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-1 text-xs font-semibold tracking-[0.22em] text-white/90 uppercase">
          Olympique Rémois Tennis de Table
        </p>

        <h1 className="text-4xl font-black tracking-tight text-balance text-white drop-shadow-sm sm:text-5xl lg:text-6xl">
        ...
        <p className="mx-auto mt-5 max-w-2xl text-base text-pretty text-white/80 sm:text-lg">
          Rejoignez un club passionné, progressez avec des entraîneurs engagés
          et participez aux tournois qui font vibrer la région.
        </p>

        <div className="mt-10">
          <Link
            href="/competitions"
            className="bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center justify-center rounded-xl px-7 py-3 text-sm font-semibold transition-transform duration-200 hover:-translate-y-0.5"
          >
